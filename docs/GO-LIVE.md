...
- Deploy edge functions: **`pay-deal`** (`docs/stripe-connect.md`), **`pay-now`** (`docs/pay-now.md` — the shareable `pay.html` widget's rail), **`stripe-webhook`** (`docs/stripe-webhook.md`, set `STRIPE_WEBHOOK_SECRET`), **`connect-onboard`** (`docs/connect-onboard.md`, JWT **OFF** — it verifies the caller's token itself; the browser preflight carries no token and would bounce if JWT were on). All use `STRIPE_SK`.
